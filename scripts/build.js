...
async function build() {
  ...
  try {
    ...
    const jsResult = await exec(`npx babel ${sourceDir} --out-dir ${jsTarget}`);

    // copy css
    process.stdout.write('Copying library style definitions... \n');
    const cssResult = await exec(`cp -r ${sourceDir}/css ${cssTarget}`);
    ...
    const heckResult = await exec(
      `npx lessc --js ${hackFileSource} ${hackFileOutputPath}`
    );
